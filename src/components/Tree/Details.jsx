...
export default function Details() {
  ...
  useEffect(() => {
    if (
      ...
      var marker = L.marker(tree.position)
        .addTo(map)
        ...
        <Card shadow="sm" padding="lg" radius="md" withBorder>
          <Stack>
            ...
            <Grid columns={2} align="flex-start" justify="flex-start">
              <Grid.Col span={{ md: 2, lg: 1 }}>
                <Title order={3}>Details</Title>
                <Text>
                  <strong>Location:</strong> {tree.location}
                </Text>
                <Text>
                  <strong>Type:</strong> {tree.type?.commonNames[0]}
                </Text>
                <Text>
                  <strong>Created on:</strong>{" "}
                  {dayjs(tree.created_at).format("DD MMMM YYYY")}
                </Text>
              </Grid.Col>
              <Grid.Col span={{ md: 2, lg: 1 }}>
                <Title order={3} mb="xs">
                ...
                <Button
                  ...
                  size="lg"
                  leftSection={
                    <Avatar src={user.avatar} radius="lg" size={30} />
                  }
                  styles={{ rightIcon: { marginLeft: "auto" } }}
                  radius="md"
                  onClick={() => navigate("/profile/" + user.id)}
                >
                  <Stack align="flex-start" justify="center" gap={0} p="sm">
